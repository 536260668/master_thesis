...
提示词操纵攻击通过精心设计的恶意输入绕过模型的安全约束，迫使模型产生违背其原始训练目标的输出。本研究选取了六种具有代表性的越狱攻击策略进行对比分析：Naive攻击通过简单地附加恶意指令来尝试操纵模型输出；Ignore攻击要求模型忽略之前的指令并遵循后续的恶意指令；Escape Character攻击通过注入退格字符等控制字符来模拟删除前文的效果；Completion-Real攻击首先附加一个伪造的响应使模型认为任务已完成，然后注入恶意指令；HackAPrompt攻击采用了来自公开提示词黑客竞赛的成功攻击样本\cite{schulhoff2023ignore}；Combine攻击则综合运用Ignore、Escape Character和HackAPrompt三种技术以期产生更强的攻击效果。实验在AlpaCare、Asclepius、BioMistral、BioLlama3四个医疗专用模型与Llama2、Llama3两个通用基线模型上进行测试，具体结果如表\ref{tab:jailbreak_strategies}所示。
...
\midrule
Naive & 32.14 & 25.37 & 19.86 & 13.72 & 10.24 & 5.23 \\
Ignore & 75.47 & 64.27 & 47.24 & 22.06 & 15.27 & 9.01 \\
Escape Character & 97.43 & 92.54 & 81.79 & 30.08 & 21.23 & 11.27 \\
Completion-Real & 99.56 & 90.87 & 78.64 & 26.35 & 17.83 & 10.42 \\
HackAPrompt & 58.01 & 50.03 & 41.73 & 18.01 & 11.98 & 5.99 \\
Combine & 99.82 & 96.74 & 90.85 & 34.27 & 24.86 & 13.94 \\
\bottomrule
\end{tabular}
}
\end{table}

从模型维度来看，六个模型的六策略平均ASR呈现出清晰的梯度分布：AlpaCare（77.07\%）$>$ Asclepius（69.97\%）$>$ BioMistral（60.02\%）$>$ BioLlama3（24.08\%）$>$ Llama2（16.90\%）$>$ Llama3（9.31\%）。医疗专用模型群体的脆弱性整体高于通用模型，其中AlpaCare、Asclepius和BioMistral三者的均值均超过60\%，而Llama2与Llama3分别仅为16.90\%和9.31\%。这种系统性差距表明，医疗领域微调过程在提升专业问答能力的同时，可能以牺牲安全对齐强度为代价。值得注意的是，同属LLaMA架构体系的AlpaCare与BioLlama3之间存在约53个百分点的ASR差距，而BioMistral虽基于Mistral架构但脆弱性仍高于BioLlama3，说明训练策略（指令微调 vs.\ 继续预训练 vs.\ 数据集微调）对安全性的影响可能大于基座架构本身。

从攻击策略维度来看，六种方法按平均ASR排序为：Combine（60.08\%）$>$ Escape Character（55.72\%）$>$ Completion-Real（53.95\%）$>$ Ignore（38.89\%）$>$ HackAPrompt（30.96\%）$>$ Naive（17.76\%）。Combine策略融合了Ignore、Escape Character和HackAPrompt三种技术，在所有模型上均取得最高或接近最高的ASR，其在AlpaCare上达到99.82\%，在BioMistral上达到90.85\%，即使在防御能力最强的Llama3上仍有13.94\%的成功率，证实了多向量协同攻击对单一防御机制的压倒性优势。Escape Character和Completion-Real两种策略在AlpaCare与Asclepius上的ASR均超过90\%，二者的平均效果相近（55.72\% vs.\ 53.95\%），但底层机制不同：前者通过注入控制字符来``擦除''先前指令，后者通过伪造完成标志来重置模型的任务上下文。在BioLlama3和Llama系列模型上，Escape Character的ASR一致高于Completion-Real（差距约3$\sim$4个百分点），提示控制字符注入在对齐较强的模型上比语义欺骗更具穿透力。

相较于上述高阶策略，Naive和HackAPrompt的攻击效力受限于策略复杂度。Naive策略仅执行简单的恶意指令附加，在Llama3上仅获得5.23\%的ASR，然而即便如此，其在AlpaCare上仍达到32.14\%，揭示了该模型几乎不具备对最基础攻击形式的过滤能力。HackAPrompt策略源自公开竞赛的成功案例集合\cite{schulhoff2023ignore}，其效果介于Naive与Ignore之间，在AlpaCare上为58.01\%，在Llama3上为5.99\%。该策略并非针对医疗模型定制，因此其在医疗场景下的效果弱于Ignore（均值差约8个百分点）这一结果符合预期。Ignore策略通过显式要求模型忽略先前指令来实施攻击，其平均ASR（38.89\%）虽不及Escape Character和Completion-Real，但在AlpaCare上仍达到75.47\%，表明简单的指令覆写对安全对齐薄弱的模型已具备较高的威胁性。

综合来看，攻击策略的有效性与模型的安全对齐强度存在交互效应。对于安全机制较弱的模型（如AlpaCare），即使低复杂度的Naive策略即可造成超过30\%的ASR；而对于安全对齐较强的模型（如Llama3），仅Combine策略能突破10\%的ASR阈值。这一在安全谱系两端观察到的不对称模式表明，防御资源应优先投入到针对组合式攻击向量的检测与拦截中，同时医疗专用模型在领域适配训练阶段应引入对抗性样本以弥补安全对齐的退化。
...
实验结果揭示了提示词长度对攻击效果的影响呈现出显著的策略依赖性，三种攻击方法表现出截然相反的长度-效果响应模式。如表\ref{tab:prompt_length_hackaprompt}所示，HackAPrompt攻击的ASR随提示词长度的增加呈单调递增趋势，六个模型在Short至Extended区间内均表现出一致的正相关关系。以AlpaCare模型为例，其ASR从Short条件下的52.74\%逐步上升至Extended条件下的63.42\%，增幅约10.68个百分点；防御能力最强的Llama3同样呈现相同趋势，ASR从3.82\%升至8.27\%，相对增幅超过一倍。这表明HackAPrompt攻击样本的信息密度随长度增加而提升，更长的攻击模板能够携带更丰富的语义操纵信号，从而有效绕过模型的安全对齐机制。

与HackAPrompt形成鲜明对比的是，Ignore攻击和Escape Character攻击均呈现出ASR随提示词长度增加而单调递减的趋势（表\ref{tab:prompt_length_ignore}与表\ref{tab:prompt_length_escape}）。在Ignore攻击中，AlpaCare的ASR从Short条件下的83.24\%下降至Extended条件下的66.81\%，降幅达16.43个百分点；Llama2的降幅更为剧烈，从20.74\%降至9.16\%，相对下降超过55\%。Escape Character攻击同样遵循这一规律，AlpaCare从99.86\%降至92.85\%，而Llama3从15.76\%降至6.95\%。这两类攻击的核心机制均依赖于通过特定指令或控制字符来覆写模型原有的任务上下文，当提示词长度增加时，额外的冗余文本可能对模型的注意力分配产生稀释效应，导致恶意指令在输入序列中的相对显著性下降，攻击效力随之减弱。

上述分歧的根源在于三种攻击的载荷构成存在本质差异。HackAPrompt攻击的载荷来源于公开竞赛中经过筛选的成功攻击案例\cite{schulhoff2023ignore}，长度的增加意味着攻击者有更多空间构建复杂的多层级语义陷阱，因此信息量与攻击力同步增长。而Ignore与Escape Character攻击的载荷在语义上高度重复——前者通过不同措辞反复要求模型忽略先前指令，后者则在指定范围内随机生成控制字符序列。这种重复填充并未引入新的攻击信号，反而因冗余的积累降低了核心恶意指令的注意力占比。

从模型维度来看，六个模型在三种攻击下的绝对ASR虽然存在量级差异，但对提示词长度变化的响应方向高度一致。AlpaCare在所有攻击方法和长度条件下均保持最高的ASR，而Llama3始终最低，两者之间的差距在Escape Character攻击的Short条件下最大（99.86\% vs.\ 15.76\%，相差84.10个百分点），在HackAPrompt攻击的Short条件下最小（52.74\% vs.\ 3.82\%，相差48.92个百分点）。这一观察表明，提示词长度作为攻击参数对模型间的相对脆弱性排序不产生影响，模型的安全对齐强度在不同长度条件下表现出稳定的层级结构。

从防御角度出发，上述发现具有差异化的启示意义。对于Ignore和Escape Character等指令覆写类攻击，提示词长度约束并非有效的防御手段，因为此类攻击在短提示词条件下已具备最高的穿透力；防御重点应放在识别和拦截显式的指令覆写模式和异常控制字符序列上。对于HackAPrompt等基于成功案例的复合型攻击，限制输入提示词的长度可在一定程度上降低攻击成功率，但由于该类攻击在短提示词条件下仍具备相当的效力（AlpaCare上为52.74\%），单纯的长度过滤不足以构成可靠的防御屏障，仍需与语义级别的恶意意图检测机制相结合。
...
实验结果揭示了注入位置对攻击效果的显著影响，且这种影响在不同安全对齐水平的模型之间表现出差异化的响应模式。如表\ref{tab:prompt_position}所示，对于四个医疗专用模型（AlpaCare、Asclepius、BioMistral和BioLlama3），Suffix注入在所有实验对象上均取得最高的ASR，其数值分别为77.07\%、69.97\%、60.02\%和24.08\%，显著优于Prefix注入的46.82\%、39.74\%、34.56\%和13.28\%。Suffix相对于Prefix的ASR增幅在四个医疗模型上分别为30.25、30.23、25.46和10.80个百分点，表明末尾注入所带来的攻击增益几乎不受模型绝对脆弱性水平的影响，呈现出跨模型的一致性。这一规律与Transformer架构中自回归解码机制的特性相吻合：位于输入序列末端的词元在时序上最接近模型的生成阶段，对输出分布的调控作用更为直接，恶意指令因而更易穿透安全对齐层。Multi-Position注入的ASR在医疗模型群体中介于Prefix与Suffix之间（AlpaCare 64.93\%、Asclepius 61.58\%、BioMistral 52.17\%、BioLlama3 19.43\%），并未实现``1+1$>$2''的叠加增益，其效果反而低于单独Suffix注入约6$\sim$14个百分点。这一观察表明，当恶意指令同时出现在序列首尾两端时，模型的注意力在两个竞争性信号之间产生分裂，Prefix位置的注入反而稀释了Suffix位置的触发效力，导致整体攻击效果的削弱。

两个通用基线模型（Llama2和Llama3）在位置响应模式上呈现出与医疗模型不同的特征。Llama2和Llama3在Multi-Position条件下的ASR分别为21.36\%和11.85\%，均高于各自的Suffix条件（16.90\%和9.31\%）和Prefix条件（10.74\%和5.27\%）。这一反转现象表明，经过充分RLHF对齐的通用模型对单一位置的恶意注入已建立起较为有效的防御屏障，但当攻击信号从多个位置同时施压时，模型的安全机制面临更大的协调负荷，防御效果出现下降。换言之，通用模型的安全对齐在应对分布式攻击向量时暴露出结构性缺陷。

从模型间的纵向比较来看，无论采用何种注入位置，六个模型的ASR排序始终保持一致的梯度分布。以Suffix条件为锚定参照，AlpaCare（77.07\%）$>$ Asclepius（69.97\%）$>$ BioMistral（60.02\%）$>$ BioLlama3（24.08\%）$>$ Llama2（16.90\%）$>$ Llama3（9.31\%），该排序与表\ref{tab:jailbreak_strategies}中六策略均值的排序完全吻合。这表明注入位置作为攻击参数并不改变模型间的相对脆弱性层级，模型的内在安全对齐强度是决定绝对ASR的主导因素。

从防御角度出发，上述发现提供了具有针对性的设计启示。对于安全对齐薄弱的医疗专用模型，防御重点应置于对输入序列末尾内容的语义审查与过滤上，因为Suffix注入在此类模型上已构成最大威胁向量。对于安全对齐较强的通用模型，防御机制则需额外关注多位置协同注入的攻击形式，避免分布式恶意信号绕过单点防御逻辑。此外，Multi-Position注入在医疗模型上效果弱于Suffix但在通用模型上效果最强这一不对称发现，意味着单一的位置过滤策略无法同时适用于不同安全水平的模型，防御方案需根据模型的对齐特征进行差异化适配。
...
表\ref{tab:attack_frequency}呈现了六个模型在1至7次重复注入条件下的平均ASR变化趋势。从整体格局来看，四个医疗专用模型（AlpaCare、Asclepius、BioMistral、BioLlama3）均呈现出ASR随注入频率增加而单调递减的规律，而两个通用模型（Llama2、Llama3）则表现出先微幅上升后持续下降的非单调响应模式，两类模型群体之间存在本质性的行为分歧。

在医疗专用模型群体中，AlpaCare虽然在单次注入下拥有最高的ASR（77.07\%），但其衰减速率同样最为剧烈：从1次到2次注入，ASR骤降16.73个百分点（77.07\%$\rightarrow$60.34\%），7次注入时进一步下降至30.82\%，累计降幅达46.25个百分点，相对衰减率约60\%。Asclepius的初始ASR略低于AlpaCare（69.97\%），但其衰减曲线显著平缓，相邻频率间的ASR降幅稳定在4$\sim$6个百分点，7次注入后仍维持42.18\%的较高水平，累计降幅仅为27.79个百分点，相对衰减率约40\%。这一差异表明Asclepius的脆弱性具有更强的持久性，即使面对重复注入仍难以通过上下文饱和效应获得有效防御。BioMistral的衰减模式介于二者之间，初始ASR为60.02\%，前两次注入间的降幅最大（15.74个百分点），此后衰减速率逐渐放缓，7次注入时稳定在19.27\%。BioLlama3的绝对ASR最低（24.08\%$\rightarrow$10.75\%），衰减幅度亦较为温和，但始终保持单调递减趋势。

Llama2和Llama3两个通用模型呈现出与医疗专用模型截然不同的频率响应特征。Llama2在单次注入下的ASR为16.90\%，2次注入时反升至18.14\%（增幅1.24个百分点），3次注入时仍维持在17.53\%的较高水平，此后方开始持续下降，7次注入时降至10.13\%。Llama3同样出现先升后降的转折：ASR从1次注入的9.31\%上升至2次注入的10.02\%，随后在3次注入时回落至9.57\%，此后单调递减至7次时的5.23\%。这种非单调模式提示，通用模型经过RLHF安全对齐训练后，其防御机制并非在首次注入时即达到最佳状态。低频重复注入可能通过提供更丰富的语境线索帮助攻击绕过初始防御屏障，但当注入频率超过某一阈值后，上下文冗余和模式重复检测机制开始发挥主导作用，ASR随之下降。

从衰减动力学的角度进一步分析，四个医疗专用模型从1次到7次注入的相对衰减率分别为：AlpaCare 60.0\%、BioMistral 67.9\%、BioLlama3 55.4\%、Asclepius 39.7\%。BioMistral的相对衰减率最高，表明其防御机制对重复注入的响应最为敏感；Asclepius的相对衰减率最低，意味着该模型的脆弱性在重复攻击下最为顽固。对于Llama2和Llama3，若以其峰值ASR（分别为18.14\%和10.02\%）为基准计算，从峰值到7次注入的相对衰减率分别为44.2\%和47.8\%，与医疗专用模型处于同一量级，表明一旦越过初始上升阶段，通用模型与医疗专用模型遵循相似的衰减规律。

综合来看，单次注入在所有医疗专用模型上均构成最大威胁，重复注入的边际收益呈显著递减趋势。然而，通用模型在低频重复注入下反而暴露出更高的脆弱性，这一发现对防御策略的设计具有重要启示：针对医疗专用模型，防御资源应优先集中于对首次注入的拦截与检测；而针对经过安全对齐训练的通用模型，还需额外关注2$\sim$3次低频重复注入场景下的安全性退化风险。
